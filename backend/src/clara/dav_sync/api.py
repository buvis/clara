...
async def create_account(body: DavSyncAccountCreate, svc: DavSvc):
...
async def list_accounts(svc: DavSvc):
...
async def get_account(account_id: uuid.UUID, svc: DavSvc):
