...
async def get_reminder(reminder_id: uuid.UUID, svc: ReminderSvc):
...
async def create_reminder(body: ReminderCreate, svc: ReminderSvc):
...
async def delete_reminder(reminder_id: uuid.UUID, svc: ReminderSvc):
