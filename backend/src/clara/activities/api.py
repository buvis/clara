...
async def create_activity_type(body: ActivityTypeCreate, svc: TypeSvc):
...
async def delete_activity_type(type_id: uuid.UUID, svc: TypeSvc):
...
async def get_activity(activity_id: uuid.UUID, svc: ActivitySvc):
    return ActivityRead.model_validate(await svc.get_activity(activity_id))


@router.post("", response_model=ActivityRead, status_code=201)
async def create_activity(body: ActivityCreate, svc: ActivitySvc):
...
async def delete_activity(activity_id: uuid.UUID, svc: ActivitySvc):
